...
export function allowedOrigin(origin) {
  ...
  // Branch previews of the app itself. Every one is a build of the app's
  // own repo on the same Vercel team, so a feature can be tried against
  // the real backend before it goes to main. Nothing else on vercel.app
  // matches.
  if (PREVIEW_ORIGIN.test(origin)) return origin;
  return null;
}

const PREVIEW_ORIGIN = /^https:\/\/resurface-app-git-[a-z0-9-]+-resurface\.vercel\.app$/;
...
/** An anonymous client, for the rare call that is not acting as a user. */
...
/**
 * Hard quota (e.g. 3 AI asks per question). Unlike rateLimited, a blocked
 * call does not consume another slot. Still in-memory / best-effort on
 * serverless — enough to stop casual burning of the API key.
 *
 * @returns {{ allowed: boolean, used: number, remaining: number }}
 */
export function takeQuota(bucket, id, max, windowMs = 7 * 24 * 60 * 60 * 1000) {
  const key = `${bucket}:${id}`;
  const now = Date.now();
  const recent = (buckets.get(key) || []).filter(t => now - t < windowMs);
  if (recent.length >= max) {
    buckets.set(key, recent);
    return { allowed: false, used: recent.length, remaining: 0 };
  }
  recent.push(now);
  buckets.set(key, recent);
  return { allowed: true, used: recent.length, remaining: max - recent.length };
}
