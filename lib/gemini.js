...
async function once({ apiKey, model, system, input, schema, maxOutputTokens, thinkingLevel, timeoutMs }) {
  ...
  // Optional per-attempt deadline. Without one, a single slow upstream call
  // runs until the platform kills the function, and the student sees
  // "Failed to fetch" — no status, no message — because nothing ever got to
  // reply. With one, the attempt fails as a 504, which is retried like any
  // other transient failure and, if that fails too, becomes a real error.
  let r;
  try {
    ...
    return { ok: false, status: 504, message: e?.message };
  }
 ...
 * through 1.2s + 2.5s of sleep while the student stares at "Thinking…" is
 * worse than a clean fail-and-retry.
 */
export async function callGemini(args) {
  let last = await once(args);
  const waits = args.fast ? [400] : [1200, 2500];
